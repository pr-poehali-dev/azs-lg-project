...
- `station_name` (обязательный, string) - название АЗС
...
ДанныеЗаправки.Вставить("station_name", "АЗС СОЮЗ №3");
...
```json
{
  "error": "Не указано название АЗС (station_name)"
}
```
...
3. Уменьшается баланс карты на `quantity` литров
4. Создается запись в истории операций с типом "заправка"
5. Возвращается результат операции с новым балансом
